...
while True:
    ...
    if ret == False:
        ...
        lower_green = np.array([35,80,90]) # mersivityball(60,60,60)(67,97,237) robustGreen:[35,80,90]
        upper_green = np.array([75, 255, 255]) # mersivityball(110,110,120) robustGreen: [75,255,255]
        ...
        mask = cv2.dilate(mask, kernel, iterations=1) # iterations=10 for mersivity video
        # mask = cv2.erode(mask, kernel, iterations=1)
        ...
        stacked_frames = np.vstack((stack1,stack2))
        # stacked_frames = np.hstack((original_frame,segmentation_img,green_highlighted_image,black_background))
        cv2.imshow('Results', stacked_frames)
